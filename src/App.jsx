...
/* ---- ที่เก็บข้อมูล "กลาง" ผ่านเซิร์ฟเวอร์ ----
   ทุกอย่าง (การบ้าน/ตารางสอน/ประกาศ) ถูกเก็บบน backend (server.mjs)
   ทุกคนที่เปิดเว็บจากเซิร์ฟเวอร์เดียวกันจะเห็นและบันทึกข้อมูลชุดเดียวกัน
   หมายเหตุ: ใช้ร่วมกันได้เฉพาะคนที่ต่อเข้า "เซิร์ฟเวอร์เดียวกัน" —
   ถ้ารันบนเครื่องตัวเอง (localhost) จะเห็นเฉพาะเครื่องนั้น ต้องนำเซิร์ฟเวอร์ไปโฮสต์ให้คนอื่นเข้าถึงได้ */
  ...
  window.storage = {
    get: async (key) => {
      try {
        ...
        return d && d.value != null ? { key, value: d.value } : null;
      } catch (e) { return null; }
    },
    set: async (key, value) => {
      try { await fetch("/api/store", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ key, value }) }); } catch (e) {}
      return { key, value };
    },
    delete: async (key) => {
      try { await fetch(`/api/store?key=${encodeURIComponent(key)}`, { method: "DELETE" }); } catch (e) {}
      return { key, deleted: true };
    },
    list: async (prefix = "") => {
      try { const r = await fetch(`/api/store?list=1&prefix=${encodeURIComponent(prefix)}`); const d = await r.json(); return { keys: d.keys || [] }; } catch (e) { return { keys: [] }; }
    },
  };
}
...
export default function App() {
  ...
  /* โหลดข้อมูลกลางจากเซิร์ฟเวอร์ (ใช้ทั้งตอนเปิดเว็บและรีเฟรชอัตโนมัติ) */
  const refresh = useCallback(async () => {
    // ถ้าเพิ่งบันทึกของตัวเองภายใน 3 วินาที ข้ามรอบนี้ไปก่อน กันข้อมูลกระพริบ
    ...
    try { const r = await window.storage.get("ann:list"); if (r?.value) setAnn(JSON.parse(r.value)); } catch (e) {}
  ...
  useEffect(() => {
    ...
    // รีเฟรชอัตโนมัติทุก 8 วินาที + ทุกครั้งที่กลับมาโฟกัสหน้าต่าง
  ...
  const saveHw = useCallback(async (next) => { lastWrite.current = Date.now(); setHw(next); try { await window.storage.set("hw:list", JSON.stringify(next)); } catch (e) {} }, []);
  const saveAnn = useCallback(async (next) => { lastWrite.current = Date.now(); setAnn(next); try { await window.storage.set("ann:list", JSON.stringify(next)); } catch (e) {} }, []);
  const saveSch = useCallback(async (next) => { lastWrite.current = Date.now(); setSch(next); try { await window.storage.set("sch:list", JSON.stringify(next)); } catch (e) {} }, []);
  const savePrefs = useCallback(async (p) => { lastWrite.current = Date.now(); setLeadDays(p.leadDays); setNotifOn(p.notifOn); try { await window.storage.set("prefs:app", JSON.stringify(p)); } catch (e) {} }, []);
  ...
  return (
    <div className="app">
      ...
      <div className="wrap">
        ...
        <div className="foot">สมุดการบ้าน · ข้อมูลทั้งหมดใช้ร่วมกันทุกคนผ่านเซิร์ฟเวอร์ · อัปเดตอัตโนมัติ</div>
      </div>
    </div>
  );
}
...
/* =====================================================================
   AI ASSISTANT — ปิดปรับปรุงชั่วคราว
   ===================================================================== */
function Assistant() {
  return (
    <div className="fadein" style={{ display: "grid", placeItems: "center", padding: "10px 0" }}>
      <div className="card panel" style={{ maxWidth: 460, width: "100%", textAlign: "center", padding: "40px 28px" }}>
        <div style={{ fontSize: 52, lineHeight: 1, marginBottom: 14 }}>🔧</div>
        <div style={{ fontFamily: "'Mitr',sans-serif", fontSize: 22, marginBottom: 8 }}>พี่เลี้ยง AI ปิดปรับปรุงชั่วคราว</div>
        <p style={{ color: "var(--ink-soft)", fontSize: 14.5, margin: 0 }}>
          ขออภัยในความไม่สะดวก ขณะนี้เรากำลังปรับปรุงระบบผู้ช่วย AI ให้ดีขึ้น
          <br />ระหว่างนี้ยังใช้ส่วนบันทึกการบ้าน ตารางสอน และประกาศได้ตามปกติครับ
        </p>
        <div style={{ marginTop: 18, display: "inline-flex", alignItems: "center", gap: 8, background: "var(--accent-soft)", color: "var(--accent)", fontWeight: 700, fontSize: 13, padding: "7px 14px", borderRadius: 20 }}>
          ● กำลังปรับปรุง
        </div>
      </div>
    </div>
